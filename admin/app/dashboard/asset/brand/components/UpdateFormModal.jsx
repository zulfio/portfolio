...
import { useDeleteAssetBrand, useUpdateAssetBrand } from "@/lib/hooks/assetBrand.hook";
import { useQueryClient } from "@tanstack/react-query";

function UpdateFormModal({ isUpdateModalOpen, setSelectedBrand, selectedBrand }) {
    ...
    const { mutate: mutateUpdate, isPending: isUpdating } = useUpdateAssetBrand();
    const { mutate: mutateDelete, isPending: isDeleting } = useDeleteAssetBrand();
    ...
    function handleMutateDelete() {
        mutateDelete(selectedBrand._id, {
            onSuccess: () => {
                queryClient.invalidateQueries({ queryKey: ["asset_brands"] })
    ...
    return (
        <Dialog open={isUpdateModalOpen} onClose={handleClose}>
            <DialogPanel className="overflow-visible">
                ...
                >
                    <Title className="mb-6 font-bold">Update {selectedBrand.name}</Title>
