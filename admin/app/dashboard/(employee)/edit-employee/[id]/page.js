...
import { reqGetEmployeeById } from "@/backend/employee";
...
async function UpdateEmployee({ params }) {
    const { success, employee, message } = await reqGetEmployeeById(params.id);
    if (!success) throw new Error(message);

    return (
        <div>
            <div className="mb-7 flex items-center justify-between gap-5">
                <Metric>Update Karyawan</Metric>
            ...
            <div className="lg:grid lg:grid-cols-12 lg:gap-x-5">
                ...
                <EditForm employee={JSON.stringify(employee)} />
            </div>
        </div>
    );
}
