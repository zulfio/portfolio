...
import { useCities, useProvinces, useSubdistrict } from "@/lib/hooks/address.hook";

export default function Contact({ control, register, errors, employee }) {
                ...
                render={({ field: { onChange, value } }) => (
                    <>
                        <SearchSelect
                            ...
                            value={value ? JSON.stringify(provinces?.find((province) => province.name === value)) : ""}
                        >
            ...
            <Controller
                ...
                render={({ field: { onChange, value } }) => (
                    <SearchSelect
                        ...
                        value={value ? JSON.stringify(cities?.find((city) => city.name === value)) : ""}
                    >
                        {cities?.map((city) => (
    ...
    }, [isSubmitted, control, subdistricts]);

    useEffect(() => {
        if (!employee) return;

        if (provinces) {
            const province = provinces.find((province) => province.name === employee.address.province);
            if (province) {
                setSelectedProvince(province);
            }
        }
        ...
        if (cities) {
            const city = cities.find((city) => city.name === employee.address.city);
            if (city) {
                setSelectedCity(city);
            }
        }
    }, [provinces, cities, employee]);
